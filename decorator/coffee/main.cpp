...
{
	{
		...
		auto latte = make_unique<CLatte>(TypeOfCoffee::Standart);
		// добавляем корицы
				...
				make_unique<CIceCubes>(				// | под нею - кубики льда
					make_unique<CLemon>(			// | | еще ниже лимон
						make_unique<CCinnamon>(		// | | | слоем ниже - корица
							make_unique<CLatte>(TypeOfCoffee::Standart)),	// | | |   в самом сердце - Латте
						2),							// | | 2 дольки лимона
	...
	// Подробнее рассмотрим работу MakeCondiment и оператора <<
	{
		...
		auto tea = make_unique<CTea>(SortOfTea::Ceylon);
		...
		*/
		...
		auto oneMoreLemonIceTea =
			make_unique<CTea>(SortOfTea::Ceylon)	// Берем чай
	...
	{
		auto beverage = 
			make_unique<CLatte>(TypeOfCoffee::Double)		// Наливаем чашечку латте,
	...
	{
		auto beverage = 
			make_unique<CMilkshake>(VolumeOfMilkshake::Middle)					// Наливаем молочный коктейль
